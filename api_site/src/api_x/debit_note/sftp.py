...
from api_x.zyt.vas.models import Event,SystemAssetAccountItem
...
def getlastbalance(id,userid):
	sql1 = """ select * from user_cash_balance_log where user_id = %s and id < %s order by id desc limit 1; """%(userid,id)
	s = text(sql1)
...
def get_recondate():
	first = db.session.query(DebitNoteDate.date).filter(DebitNoteDate.state==False).order_by(DebitNoteDate.date.desc()).first()
	print first
	return first
...
def download_sftp_bydate(date):
	...
	try:
		...
		sftp.get("bjlysy/201507021000395502/JYMX_201507021000395502_%s.csv"%date, "./%s.csv"%date)
		t.close();
	except Exception, e:
		import traceback
		traceback.print_exc()
...
def create_DNDetail(sn,vas_name,amount,order_id,state,valid,created_on,type):
	...
	first = db.session.query(DebitNoteDetail.sn).filter(DebitNoteDetail.sn==sn,DebitNoteDetail.state==state).first()
	if not first:
...
def lydata2db(lydata):
	for ly in lydata:
		print ly
		create_DNDetail(ly.sn,ly.vas_name,ly.amount,ly.order_id,state=ly.state,valid=ly.valid,created_on=ly.created_on,type=ly.type)
def lldata2db(lldata):
	for ll in lldata:
		print ll
		create_DNDetail(ll["sn"],ll["vas_name"],ll["amount"],ll["vas_sn"],state=True,valid=False,created_on=datetime.datetime.strptime(ll["created_on"], "%Y%m%d %H:%M:%S") ,type="PAYMENT" if ll["type"]== "0" else "REFUND")
def deletedata():
...
def lvyedata_ready(date='2015-09-10'):
	...
	for ret in result:
		...
		create_DNDetail(ret.tx_sn,ret.vas_name,ret.amount,ret.order_id,state=False,valid=False,created_on=ret.created_on,type=ttype)
