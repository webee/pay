...
def pay(vas_name, payment_entity):
    # TODO: 注册支付方式
    if vas_name == test_pay.NAME:
        return _pay_by_test_pay(payment_entity)
    elif vas_name == lianlian_pay.NAME:
        return _pay_by_lianlian_pay(payment_entity)
...
def _pay_by_test_pay(payment_entity):
...
def _pay_by_lianlian_pay(payment_entity):
...
class PaymentEntity(object):
    def __init__(self, source, user_id, user_created_on, tx_sn, tx_created_on, product_name, product_desc, amount):
        self.source = source
